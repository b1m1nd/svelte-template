...
_startup = 1,
  ...
  var s = _ref2.s,
      d2 = _ref2.d2;
  return (s = "scroll" + d2) && _isViewport(element) ? Math.max(_docEl[s], _body[s]) - (_win$2["inner" + d2] || _docEl["client" + d2] || _body["client" + d2]) : element[s] - element["offset" + d2];
},
...
_listeners = {},
  ...
  top: 0
    ...
    _parsePosition$1 = function _parsePosition(value, trigger, scrollerSize, direction, scroll, marker, markerScroller, self, scrollerBounds, borderWidth, isViewport, scrollerMax) {
...
var ScrollTrigger = /*#__PURE__*/function () {
  ...
  _proto.init = function init(vars, animation) {
    ...
    self.refresh = function (soft) {
      ...
      var size = (isViewport ? _win$2["inner" + direction.d2] : scroller["client" + direction.d2]) || 0,
      ...
      start = _parsePosition$1(parsedStart, trigger, size, direction, self.scroll(), markerStart, markerStartTrigger, self, scrollerBounds, borderWidth, isViewport, max) || (pin ? -0.001 : 0);
      ...
      end = Math.max(start, _parsePosition$1(parsedEnd || (parsedEndTrigger ? "100% 0" : max), parsedEndTrigger, size, direction, self.scroll() + offset, markerEnd, markerEndTrigger, self, scrollerBounds, borderWidth, isViewport, max)) || -0.001;
    ...
    self.enable = function () {
      if (!enabled) {
        ...
        !animation || !animation.add ? self.refresh() : gsap$1.delayedCall(0.01, self.refresh) && (change = 0.01) && (start = end = 0); // if the animation is a timeline, it may not have been populated yet, so it wouldn't render at the proper place on the first refresh(), thus we should schedule one for the next tick.
      }
    };
  ...
  ScrollTrigger.register = function register(core) {
    ...
    if (gsap$1) {
      ...
      if (_body) {
        ...
        _addListener(_doc$2, "visibilitychange", function () {
          return _doc$2.hidden || _refreshAll();
        });

        _addListener(_doc$2, "DOMContentLoaded", _refreshAll);

        _addListener(_win$2, "load", function () {
          return _lastScrollTime || _refreshAll();
        });

        _addListener(_win$2, "resize", _onResize);
      }
    }
...
function scrollMe(node, _ref2) {
  ...
  var method = ScrollTrigger.create({
    ...
    start: 'center center',
    toggleActions: 'play none none reverse' //markers: { startColor: 'green', endColor: 'red', fontSize: '18px', indent: 10 },
...
function create_fragment$1(ctx) {
  ...
  var block = {
    ...
    l: function claim(nodes) {
      var head_nodes = query_selector_all("[data-svelte=\"svelte-rfgxwv\"]", document.head);
    ...
    h: function hydrate() {
      ...
      add_location(p0, file$1, 23, 0, 626);
      add_location(p1, file$1, 24, 0, 718);
      attr_dev(h1, "class", "test");
      add_location(h1, file$1, 35, 0, 1245);
      attr_dev(div0, "class", "spacer svelte-45udk8");
      add_location(div0, file$1, 44, 0, 1426);
      add_location(p2, file$1, 46, 2, 1534);
      attr_dev(div1, "class", "pinned svelte-45udk8");
      add_location(div1, file$1, 45, 0, 1458);
      attr_dev(div2, "class", "spacer svelte-45udk8");
      add_location(div2, file$1, 48, 0, 1558);
      add_location(p3, file$1, 50, 2, 1666);
      attr_dev(div3, "class", "pinned svelte-45udk8");
      add_location(div3, file$1, 49, 0, 1590);
      attr_dev(div4, "class", "spacer svelte-45udk8");
      add_location(div4, file$1, 52, 0, 1690);
    },
    m: function mount(target, anchor) {
